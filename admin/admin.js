...
async function loadDashboardStats() {
    ...
    // Contagem de briefings ativos (status != 'Concluído')
    briefings.forEach(b => {
        if (b.status !== 'Concluído') ativos++;
    });
    ...
    // Tabela de últimos leads (últimos 5 briefings)
    const recentLeadsTable = document.getElementById('recentLeadsTable');
    recentLeadsTable.innerHTML = '';
    
    const latestBriefings = briefings.slice(-5).reverse();
    ...
    latestBriefings.forEach(b => {
        ...
        row.innerHTML = `
            ...
            <td><button class="btn btn-secondary" style="padding: 6px 12px; font-size: 12px;" onclick="openBriefingDetail('${b.id}')"><i class="fa-solid fa-eye"></i> Detalhes</button></td>
        `;
